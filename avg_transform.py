...
def bayesian_group_estimate(df, groupby_col, y_col, dist_type, train_bool=None, exclude_current_row=False,
                            prior_group=None, min_beta_size=None, min_beta_groups=None):
    """Group the dataframe by groupby_col, and calculates the Bayesian estimate of the mean of y_col in each group.
    
    Args:
        df: pd.DataFrame
        groupby_col: string, name of column to group by
        y_col: string, name of column to aggregate
        dist_type: string, one of 'beta', 'normal' or 'log_normal'. Use beta where y_col is binary, or normal 
            where y_col is continuous.
        ...
        min_beta_size: minimum size of groups used to calculate the prior variance for the beta distribution
        min_beta_groups: minimum number of groups used to cacluate the prior variance for the beta distribution
    Returns:
    ...
    if dist_type == 'log_normal':
        ...
        df_copy['log_y'] = np.log(df[y_col])
        sum_col = df_copy.loc[train_bool].groupby(groupby_col)['log_y'].sum().to_frame('sum')
        size_col = df_copy.loc[train_bool].groupby(groupby_col)['log_y'].size().to_frame('count')
        ...
        sum_col = df.loc[train_bool].groupby(groupby_col)[y_col].sum().to_frame('sum')
        size_col = df.loc[train_bool].groupby(groupby_col)[y_col].size().to_frame('count')
        if type(groupby_col) == str:
            df_col = df[groupby_col].to_frame()
        else:
            df_col = df[groupby_col]
        ...
        sample_size = pd.merge(df_col, size_col, 'left', left_on=groupby_col, right_index=True)['count']
        if exclude_current_row:
            sample_sum.loc[train_bool] -= df.loc[train_bool, y_col]
            sample_size.loc[train_bool] -= 1
        sample_mean = sample_sum / sample_size
        sample_mean.loc[sample_size == 0] = np.nan
        if prior_group is None:
            mean_prior = df.loc[train_bool, y_col].mean()
        else:
            if type(prior_group) == str:
                prior_col = df[prior_group].to_frame()
            else:
                prior_col = df[prior_group]
            sample_mean_prior = df.loc[train_bool].groupby(prior_group)[y_col].mean().to_frame('mean')
            mean_prior = pd.merge(prior_col, sample_mean_prior, 'left', left_on=prior_group, right_index=True)['mean']
            mean_prior = mean_prior.fillna(mean_prior.mean())

    if dist_type == 'beta':
        # Estimate the prior variance by taking the variance of the groups with >= min_beta_size values (including smaller
        # groups will over-estimate the variance). If there are <= min_beta_groups such groups, just use all groups.
        large_ind = sample_size >= min_beta_size
        if large_ind.sum() < min_beta_groups:
...
def group_estimate(df, groupby_col, y_col, dist_type, train_bool=None, exclude_current_row=False):
    """Group the dataframe by groupby_col, and calculates the maximum likelihood estimate of the mean of y_col in 
    each group.
    ...
    if dist_type == 'log_normal':
        ...
        sample_mean = sample_sum / sample_size
        sample_mean.loc[sample_size == 0] = np.nan
        mean_mle = df_copy.loc[train_bool, 'log_y'].mean()
    else:
        sum_col = df.loc[train_bool].groupby(groupby_col)[y_col].sum().to_frame('sum')
        size_col = df.loc[train_bool].groupby(groupby_col)[y_col].size().to_frame('count')
        if type(groupby_col) == str:
        ...
        sample_mean = sample_sum / sample_size
        sample_mean.loc[sample_size == 0] = np.nan
        mean_mle = df.loc[train_bool, y_col].mean()
...
def group_median(df, groupby_col, y_col, train_bool=None):
    """Group the dataframe by groupby_col, and calculates the median of y_col in each group.
    ...
    std = df.loc[train_bool].groupby(groupby_col)[y_col].std().to_frame('median')
    if type(groupby_col) == str:
        df_col = df[groupby_col].to_frame()
    else:
        df_col = df[groupby_col]
...
def group_std(df, groupby_col, y_col, train_bool=None):
    """Group the dataframe by groupby_col, and calculates the standard deviation of y_col in each group.
    ...
    std = df.loc[train_bool].groupby(groupby_col)[y_col].std().to_frame('std')
    if type(groupby_col) == str:
        df_col = df[groupby_col].to_frame()
    else:
        df_col = df[groupby_col]
